...
public class ContentController {
	...
	public String publicSubmit(Content content, ModelMap map){
	...
	public String editSubmit(Content content, ModelMap map){
	...
	@ResponseBody
	public AsynExecResult<Boolean> deleteContent(@RequestParam("id") int id){
	...
	/**
	 * 将卖家上传的文件保存到服务器文件夹中，并返回存储的相对路径给客户端
	 * @param file
	 * @param request
	 * @param response
	 * @return
	...
	public AsynExecResult<String> doUploadFile(@RequestParam("file") MultipartFile file,/*HttpServletRequest request,*/ HttpServletResponse response){
		...
		try {
			...
			//服务器中文件的全路径
			String filePath = imagePath + File.separator + "image" + File.separator + newFileName;
			//返回给前端的文件相对路径
